       ...
       PSI_LI *_inttype, PSI_LI *_nsize_int, PSI_LD *_DINT_cont )
{
    ...
    PSI_LD *Ca, *Cb, *expo_a,  *expo_b, *coef_a, *coef_b ;
    PSI_LD dint_prim [ _N_STV_ ], *dint_prim_tmp ; 
...
///fprintf ( stdout, "ish, jsh: %d, %d: %16.8e\n", Ish, Jsh, ABAB ) ;
    ...
    for ( ap = 0 ; ap < SHEL_TEM [ a ] ; ap++ ) {
      ...
      for ( bp = 0 ; bp < SHEL_TEM [ b ] ; bp++ ) {
        ...
        for ( inuc = 0 ; inuc < NUMB_ATOM ; inuc++ ) {
            PSI_LD Z, *Cc, *PC, *P, *PCPC ;
            Z    = ATOM_CHARG [ inuc ] ;
            Cc   = ATOM_XYZ  + 3 * inuc  ;
            PC   = coefs_stv + _ADDR_PC_ ;
            P    = coefs_stv + _ADDR_P_  ;
            PCPC = coefs_stv + _ADDR_PCPC_ ;
            for ( i = 0, *PCPC = 0 ; i < 3 ; i++ ) {
              PC [ i ]  =  P [ i ] - Cc [ i ] ;
              *PCPC    += PC [ i ] * PC [ i ] ;
            }
            int1_v_iic ( m_value, Z, coefs_stv, DINT_WORK ) ;
            int1_v_rec ( inttype, coefs_stv, DINT_WORK, &dint_prim_tmp ) ; 
            for ( i = 0 ; i < nsize_int ; i++ ) {
                dint_prim [ i ] += dint_prim_tmp [ i ] ;
            }
        }
        //------------------------------------
        // Contraction
       ...
       PSI_LD *Sint )
{
    ...
    for ( b = 0 ; b < ncomp_b ; b++ ) {
        fprintf ( stdout, "%5d%5d:%5d %24.16e\n",
                  offset_a + a, offset_b + b, iint, Sint [ iint ] ) ;
        iint++ ;
    }
    }
