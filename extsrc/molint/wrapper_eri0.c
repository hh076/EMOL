...
{
    PSI_LI i, numb_atom, numb_shell, numb_prim ;
    ...
    eri0_init ( numb_atom, numb_shell, numb_prim,
          ...
          VALUE _ish, VALUE _jsh, VALUE _ksh, VALUE _lsh )
{
    ...
    PSI_LI i, ish, jsh, ksh, lsh, inttype, nsize_int ;
    PSI_LD eris [ _N_ERI_ ] ;
    ...
    ksh = NUM2INT ( _ksh ) ;
    lsh = NUM2INT ( _lsh ) ;
    eri0_calc ( ish, jsh, ksh, lsh, &inttype, &nsize_int, eris ) ;

    _eris = rb_ary_new2 ( nsize_int ) ;
    for ( i = 0 ; i < nsize_int ; i++ ) {
        rb_ary_store ( _eris, i, rb_float_new ( eris [ i ] ) ) ;
    }
...
///
///    fprintf ( stdout, "===============================================\n" ) ;
///    fprintf ( stdout, "c: (%3d%3d,%3d%3d ):%5d%5d\n", ish, jsh, ksh, lsh, inttype, nsize_int ) ;
///    for ( i = 0 ; i < nsize_int ; i++ ) {
...
{
    VALUE _results ;
    PSI_LI i = 0, j = 0, k = 0, l = 0, li = 0, lj = 0, lk = 0, ll = 0 ;
    PSI_LI a, b, c, d ;

    eri0_exchange_to_atomic_order (
       i, j, k, l, li, lj, lk, ll, &a, &b, &c, &d ) ;

    _results = rb_ary_new2 ( 4 ) ;
    rb_ary_store ( _results, 0, INT2NUM ( a ) ) ;
    rb_ary_store ( _results, 1, INT2NUM ( b ) ) ;
    rb_ary_store ( _results, 2, INT2NUM ( c ) ) ;
    rb_ary_store ( _results, 3, INT2NUM ( d ) ) ;
    return _results ;
}
