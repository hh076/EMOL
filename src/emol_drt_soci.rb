...
class Node
  ...
  def initialize( _orb, _nve, _is, _mo )
    @orb = _orb
    @nve = _nve
    @is = _is
    @code = [ @orb, @nve, @is ]
...
#
  def generate_core_node( h_nodes )
    h_nodes.store( [0, 0, 0], Node.new( 0, 0, 0, @mo ) )  # bottom node
    for k in @mo.core_range do
      if k == 1
        h_nodes.store( [1, 0, 0], Node.new( 1, 0, 0, @mo ) )   # bottom-empty
        h_nodes.store( [1, 1, 1], Node.new( 1, 1, 1, @mo ) )   # bottom-up
        h_nodes.store( [1, 2, 0], Node.new( 1, 2, 0, @mo ) )   # bottom-full
      else
        nve = 2 * k
        h_nodes.store( [k, nve-2, 0], Node.new( k, nve-2, 0, @mo ) )
        h_nodes.store( [k, nve-2, 2], Node.new( k, nve-2, 2, @mo ) )
        h_nodes.store( [k, nve-1, 1], Node.new( k, nve-1, 1, @mo ) )
        h_nodes.store( [k, nve, 0], Node.new( k, nve, 0, @mo ) )
      end
    end
  end
#
  ...
  def generate_external_node( h_nodes )
    ...
    for k in @mo.ext_range do
      if k < @mo.ext_range.last
        h_nodes.store( [k, nve-2, sp2], Node.new( k, nve - 2, sp2, @mo ) )
        if k < @mo.ext_range.last-1
          h_nodes.store( [k, nve-2, sp2-2], Node.new( k, nve - 2, sp2 - 2, @mo ) ) 
          h_nodes.store( [k, nve-2, sp2+2], Node.new( k, nve - 2, sp2 + 2, @mo ) )
        end 
        h_nodes.store( [k, nve-1, sp2-1], Node.new( k, nve - 1, sp2 - 1, @mo ) )
        h_nodes.store( [k, nve-1, sp2+1], Node.new( k, nve - 1, sp2 + 1, @mo ) )
      end
      h_nodes.store( [k, nve, sp2], Node.new( k, nve, sp2, @mo ) )
    end
    if @mo.ext_range.last < @mo.ext_range.first # no external の場合 top
      # node を格納
      h_nodes.store( [@mo.ext_range.last, nve, sp2], Node.new( @mo.ext_range.last, nve, sp2, @mo ) )
    end
  end
#
...
#
  def generate_valence_node(orb, nve, is, dest, h_nodes)
    new_node = Node.new(orb, nve, is, @mo)
