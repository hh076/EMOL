...
#require_relative 'emol_drt_foci'
...
#           br_symbol.expr.show
        ...
        if ( obj.instance_of?( Array ) ) then   #
            ...
            @coef = obj[ 5 ].clone
            self.show
        elsif ( obj.instance_of?( Symbolic_expression ) ) then
            @norb = obj.norb
    ...
    def extract_1el_expression
        ...
        return Symbolic_expression.new( [ @norb, @csf_list, ij_1el, addr_1el, pqrs_1el, coef_1el ] )
    end
...
symbol = Symbolic_expression.new( molinfo )
symbol_1el = symbol.extract_1el_expression

#####################################################################
modulename_data_expr  = "Data_symbolic_expr"
filename_expr  = modulename_data_expr
fwrite_multiobject( [ symbol ], [ "symbolic_expr" ], 
                    modulename_data_expr, filename_expr )
#####################################################################

#####################################################################
modulename_data_expr  = "Data_symbolic_expr_1el"
filename_expr  = modulename_data_expr
